...
int test_q8_matvec() {
    ...
    float x[32]; for (float& v : x) v = 1.f; float y = 0.f;
...
int test_q4_matvec_and_embedding() {
    ...
    float row[32]{}; nedo::kernels::embedding_row(t, w, 0, row);
    for (int i = 0; i < 16; ++i) if (!near(row[i], float(i - 8)) || !near(row[i + 16], float(7 - i))) return 5;
    return 0;
}
}
int main() {
